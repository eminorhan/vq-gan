...
class ImageLogger(Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        ...
        # apologize for hackiness below
        if trainer.global_step % 10000 == 0:
            if trainer.global_rank == 0:
                print("Summoning checkpoint. Global step:", trainer.global_step)
                ckpt_path = os.path.join(trainer.callbacks[3].dirpath, "last.ckpt")  # very hacky
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Learning rate: {:.2e} = {} (accumulate_grad_batches) * {} (num_gpus) * {} (batchsize) * {:.2e} (base_lr)".format(model.learning_rate, accumulate_grad_batches, ngpu, bs, base_lr))
